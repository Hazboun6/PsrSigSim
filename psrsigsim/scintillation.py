...
class phase_screen:
    def __init__(self, signal_object, scint_param_model='SC', Freq_DISS=None,
        ...
        variance with spectral index, spectral_ind and inner and outer scales
        as specified for a sample interval dx.

        C_n^2, the power-law amplitude of electron-density wavenumber spectrum,
        is calculated seperately by the cnsq_calc() method.
        ...
        self.r_Fresnel = np.sqrt(c / (max_freq * 1e6) * (D_screen * KPC_TO_M)
                                 / (2.0*np.pi))
...
def cnsq_calc(nu=1000, dnud=None, taud=None, dtd=None, D=1,
              PM=None, beta=11./3., mode='screen', ds=0.001):
    '''
    Function for calculating the electron-density wavenumber
    ...
    Outputs:
    cnsq = C_n^2, the power-law amplitude of electron-density
           wavenumber spectrum
    '''
...
Be careful with float division now. This has been removed to allow for numpy
